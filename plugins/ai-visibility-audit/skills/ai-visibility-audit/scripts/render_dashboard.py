...
.risk{background:linear-gradient(180deg,#2a1414,#1a0f0f);border:1px solid #4a2020;border-left:3px solid var(--bad);border-radius:10px;padding:16px 18px;margin:0 0 24px}
.risk h2{color:var(--bad);margin:0 0 8px;font-size:14px;text-transform:uppercase;letter-spacing:.06em}
.risk .meta{color:var(--muted);font-size:12.5px;margin:0 0 8px}
.risk .summary{font-size:14px;margin:0 0 10px}
.risk .themes{margin:0;padding-left:18px;font-size:13px;color:var(--ink)}
.risk .themes li{margin:0 0 4px}
...
<div class="risk" id="risk-sec" style="display:none">
  <h2>⚠ Reputation risk found</h2>
  <div class="meta" id="risk-meta"></div>
  <p class="summary" id="risk-summary"></p>
  <ul class="themes" id="risk-themes"></ul>
</div>
<div class="grid">
...
if(M.reputation_risk){
  const rr=M.reputation_risk;
  document.getElementById('risk-sec').style.display='';
  document.getElementById('risk-meta').textContent=`${esc(rr.platform||"")} · ${rr.rating!=null?rr.rating+"/5":""}${rr.review_count!=null?" from "+rr.review_count+" reviews":""}`;
  document.getElementById('risk-summary').textContent=rr.summary||"";
  document.getElementById('risk-themes').innerHTML=(rr.themes||[]).map(t=>`<li>${esc(t)}</li>`).join('');
}
